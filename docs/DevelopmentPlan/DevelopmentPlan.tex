...
The roles are not fixed, since workload, availability, and project needs will change over the term.
  ...
  \item The computer vision system may not process images at a sufficient frame rate.
        \begin{itemize}
          \item If this occurs, we will optimize the existing model, consider using a more
                ...
                lower the frame rate, or switch to traditional algorithmic methods
                (e.g., frame differencing, background subtraction, optical flow).
          \item We also reserve the option to use models trained for a narrower class of targets
                (e.g., rockets only) to increase throughput.
        \end{itemize}

  \item The STM32 may not deliver real-time control to the camera.
        \begin{itemize}
          \item If this occurs, we will optimize the control firmware and scheduling, migrate to a
                higher-performance MCU (e.g., \textit{STM32H7} or \textit{Teensy 4.1}),
                and/or offload time-critical loops to dedicated motor drivers with hardware timers.
        \end{itemize}

  \item Integration of the frontend, backend, computer vision model, and microcontroller may be more difficult than expected.
        \begin{itemize}
          \item If this occurs, we will consider using a more powerful integrated SBC such as the
                \href{https://developer.nvidia.com/embedded/jetson-xavier-nx}{Jetson Xavier NX},
                deploying selected components via cloud computation if network conditions allow,
                or redesigning the control flow to reduce coupling and local processing load.
        \end{itemize}
\end{enumerate}
