...
utilizes computer vision algorithms to detect and follow the rocket. We also desgined a 
...
\begin{itemize}
    \item \textbf{Performance:} sustained \(\geq 60\) FPS video, \(\grq 1080p\) resolution, real-time streaming. 
    \item \textbf{Reliability \& Safety:} emergency stop all movements. 
    \item \textbf{Usability:} operator can arm, acquire, and track in \(\leq 4\) clicks; clear status prompts.
    \item \textbf{Out of scope:} operate under extreme environment. Third party usage.
\end{itemize}
