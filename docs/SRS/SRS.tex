...
\begin{tabularx}{\textwidth}{llX}
  ...
  Nov. 16, 2025            & Jianqing Liu            & Fix Peer Review Comments \\
  \bottomrule
\end{tabularx}
...
\begin{itemize}[leftmargin=*]
  ...
  \item[SC-2] \emph{The gimbal control hardware shall use an STM32-family
          microcontroller board supplied or approved by the McMaster Rocketry Team.}\\[2mm]
        \textbf{Rationale:} The Rocketry Team already provides STM32-based hardware and tooling, and the project must integrate with this ecosystem.\\
        \textbf{Fit Criterion:} All low-level gimbal control firmware runs on an STM32 device from the approved list.

  \item[SC-3] \emph{The vision and tracking pipeline shall run on an NVIDIA
          Jetson Orin Nano, or an equivalent NVIDIA Jetson-class embedded GPU
          module.}\\[2mm]
        \textbf{Rationale:} The Jetson platform is provided to the team and is the supported embedded GPU for this project.\\
        \textbf{Fit Criterion:} The deployed system executes the full vision pipeline on a Jetson Orin Nano (or approved equivalent) and meets the performance requirements in Section~\ref{sec:performance}.

  \item[SC-4] \emph{The project shall reuse open-source components for computer
          vision, deep learning, media streaming/recording, containerization, and
          continuous integration, provided they meet the performance and
          verification criteria defined in Section~\ref{sec:reusable-components}.}\\[2mm]
        \textbf{Rationale:} Using established open-source tools reduces risk and development effort.\\
        \textbf{Fit Criterion:} The selected components for these roles are open-source and have recorded verification results linked to the requirements they support.
\end{itemize}

\subsection{Implementation Environment of the Current System}

The current manual recording process uses consumer or semi-professional cameras
and tripods, with no standardized hardware platform. The mandated hardware
platform for RoCam itself is captured in the solution constraints (SC-2--SC-3).
...
The system is expected to rely on open-source off-the-shelf software for
computer vision, deep learning, streaming/recording, containerization, and
continuous integration. The requirement to reuse such components is captured in
solution constraint SC-4, while Section~\ref{sec:reusable-components} lists the
intended reusable components and their verification approach.
...
\begin{itemize}[leftmargin=*]
  ...
  \item[FR-8] \emph{The system must begin outputting video as soon as it is
          initialized.}\\[2mm]
        \textbf{Rationale:} Immediate video output ensures that event video is available to stakeholders from the earliest possible moment.\\
        \textbf{Fit Criterion:} Upon system initialization, a connected display immediately receives a live video signal from the camera without requiring additional user action.\\
        \textbf{Source PUC:} 1 \\
        \textbf{Priority:} High

  \item[FR-9] \emph{The video output must have a text overlay that displays the current
          gimbal tilt and pan angle.}\\[2mm]
        ...
        \textbf{Fit Criterion:} The video output includes a text label showing current tilt and pan angles, updating once per video frame.\\
        \textbf{Source PUC:} 1 \\
...
\begin{itemize}[leftmargin=*]
  ...
  \item[SLR-3] \emph{The system shall be able to output 1080p 60fps video to a
          connected display}\\[2mm]
        \textbf{Rationale:} The connected display is the output of the system\\
        \textbf{Fit Criterion:} The system is able to output 1080p 60fps video to a connected display.\\
        \textbf{Source PUC:} 1 \\
        \textbf{Priority:} High
\end{itemize}
...
\begin{itemize}[leftmargin=*]
  ...
  \item[SCR-3] \emph{The system must be remote operatable over a local network.}\\[2mm]
        ...
        \textbf{Fit Criterion:} The system should be able to perform all the product use cases over a local network with no physical interaction at the camera site once deployed.\\
        \textbf{Source PUC:} all \\
        \textbf{Priority:} High
...
This section is not applicable, as this is a one-time capstone project.
...
\begin{itemize}[leftmargin=*]
  \item[AUR-1] \emph{The system shall log all operations performed by the user,
...
None, there are no standards compliance requirements for this project.
...
\noindent\textbf{Note:} Platform choices such as the STM32 microcontroller and the
NVIDIA Jetson Orin Nano are documented in the solution constraints (SC-2 and
SC-4). This section focuses on the open-source components we plan to reuse and
...
This section describes the procedure for migrating existing workflow to use the
system we developed.

\subsection{Requirements for Migration to the New Product}

\begin{enumerate}
  \item The client need to select a place with power source to deploy the system.
  \item The client need to set up a local network between the operator and the system.
\end{enumerate}

\subsection{Data That Has to be Modified or Translated for the New System}

None, as the system will only be producing new video footage and logs, it does
not need to process any existing data.
