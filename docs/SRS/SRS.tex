...
The list below shows all business events happening in the real world that
affect the work of tracking rockets.

\begin{enumerate}[label=BUC \arabic*., wide=0pt, leftmargin=*]
  \item {\bf Business Event:} Live stream equipment is ready\\[2mm]
        {\bf Inputs:} Vocal confirmation from live stream technician\\
        {\bf Outputs:} Start transmitting the video to the live stream\\
        {\bf Summary of BUC:} Start transmitting the video to the live
        stream once the live stream equipment is ready.\\
  \item {\bf Business Event:} Launch countdown starts\\[2mm]
        {\bf Inputs:} Vocal count down from Mission Control\\
        {\bf Outputs:} Start recording the rocket flight\\
        {\bf Summary of BUC:} Start tracking the rocket once the countdown begins.\\
  \item {\bf Business Event:} Rocket takes flight\\[2mm]
        {\bf Inputs:} Video captured from camera\\
        {\bf Outputs:} Keeps pointing the camera to the rocket\\
        {\bf Summary of BUC:} Keep tracking the rocket once it takes flight.\\
  \item {\bf Business Event:} Rocket completes flight\\[2mm]
        {\bf Inputs:} Video captured from camera\\
        {\bf Outputs:} Stop tracking the rocket; save the video; update mission control on flight status\\
        {\bf Summary of BUC:} Stop tracking the rocket once it completes flight.\\
  \item {\bf Business Event:} Flight performance analyst wants to view the footage\\[2mm]
        {\bf Inputs:} Request from Flight performance analyst\\
        {\bf Outputs:} Exported video files and logs\\
        {\bf Summary of BUC:} Provide the footage to the flight performance analyst.\\
\end{enumerate}

\subsection{Specifying a Business Use Case (BUC)}

\textbf{BUC Scenario for BE 5}
...
\begin{enumerate}[label=PUC \arabic*., wide=0pt, leftmargin=*]
  \item {\bf Name:} Record Footage\\[2mm]
        {\bf Actors:} Camera Operator, Camera\\
        {\bf Inputs:} Start / stop commands from camera operator, video feed from camera\\
        {\bf Outputs:} Recorded video footage and logs\\
        {\bf BUCs:} BUC 2, BUC 4, BUC 5\\
  \item {\bf Name:} Live Video Feed\\[2mm]
        {\bf Actors:} Camera Operator, Camera, Live Stream Technician\\
        {\bf Inputs:} Start / stop commands from camera operator, video feed from camera\\
        {\bf Outputs:} Live video feed to Live Stream Technician\\
        {\bf BUCs:} BUC 1\\
  \item {\bf Name:} Control Gimbal\\[2mm]
        {\bf Actors:} Camera, Gimbal\\
        {\bf Inputs:} Video feed from camera\\
        {\bf Outputs:} Movement commands to gimbal\\
        {\bf BUCs:} BUC 3\\
\end{enumerate}
